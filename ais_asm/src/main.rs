...
fn dump_offset(asm: &mut DynAsm) -> Result<(), TopError> {
    ...
    for i in 0..32 {
        ...
        let mut instr = Instruction::xlead(eax,r0,Offset::Number(0),ais::AddrSize::Bits32, Size::Bits32);
...
fn dump_constant(asm: &mut DynAsm) -> Result<(), TopError> {
    ...
    for i in 0..32 {
        ...
        let mut instr = Instruction::xaluir(SubOpXalu::ADD,DpCntl::Word,eax,r0, ais::Const::Raw(i));
...
fn test_eflags(asm: &mut DynAsm) -> Result<(), TopError> {
    ...
    for (a, b) in comb {
        ...
        asm.gen(Instruction::xalur(SubOpXalu::ADD, DpCntl::Word, eax, r4, r5))?;
        asm.gen(Instruction::xalur(SubOpXalu::ADD, DpCntl::Word, eax, r4, r5))?;
        asm.gen(Instruction::xalur(SubOpXalu::ADD, DpCntl::Word, eax, r4, r5))?;
        asm.gen(Instruction::xalur(SubOpXalu::ADD, DpCntl::Word, eax, r4, r5))?;
        asm.gen(Instruction::xalur(SubOpXalu::ADD, DpCntl::Word, eax, r4, r5))?;
...
fn test_xj(asm: &mut DynAsm) -> Result<(), TopError> {
    ...
    for (a,b) in comb {
        ...
        for i in 0..16 {
            ...
            xj.mask = 6 << 11 | i << 2;
            asm.gen(xj)?;
            asm.gen(Instruction::xalur(SubOpXalu::OR, DpCntl::Word, eax, eax, ecx))?;
